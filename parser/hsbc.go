...
import (
	...
	"bitbucket/api_format/internal/models"
)
...
func isTerminalTransaction(desc string) bool {
	...
	return strings.Contains(upper, "BALANCE CARRIED FORWARD") ||
		strings.Contains(upper, "BALANCE BROUGHT FORWARD") ||
	...
	for i := 0; i < len(lines); i++ {
		...
		if hasDate && i+1 < len(lines) {
			...
			if nextLine != "" && !startsWithDate(nextLine) {
				...
				if txn, ok := p.tryDateWithContinuation(line, nextLine); ok {
					txn.ParseMethod = "tab-separated-joined"
					transactions = append(transactions, txn)
					dl.Result = "parsed"
					dl.Method = "tab-joined"
...
func (p *HSBCParser) tryDateWithContinuation(line, nextLine string) (models.Transaction, bool) {
	...
	baseDesc := strings.TrimSpace(line[dateIdx+len(date):])
	if baseDesc == "" {
		return models.Transaction{}, false
	}

	next := strings.TrimSpace(nextLine)
	if next == "" || startsWithDate(next) {
		return models.Transaction{}, false
	}

	amountLocs := trailingAmountsPattern.FindAllStringIndex(next, -1)
	if len(amountLocs) == 0 {
		return models.Transaction{}, false
	}

	prefix := strings.TrimSpace(next[:amountLocs[0][0]])
	description := strings.TrimSpace(baseDesc + " " + prefix)
	for strings.Contains(description, "  ") {
		description = strings.ReplaceAll(description, "  ", " ")
	}

	amountMatches := trailingAmountsPattern.FindAllStringSubmatch(next, -1)
	if len(amountMatches) == 0 {
		return models.Transaction{}, false
	}

	amounts := make([]float64, 0, len(amountMatches))
	...
	if strings.HasPrefix(upperDesc, "CR ") || strings.Contains(upperDesc, "INTEREST") || isCreditDescription(description) {
		txn.Type = "CREDIT"
